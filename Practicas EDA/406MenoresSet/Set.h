...
public:
    ...
    void add(T const& elem) {
        bool found;
        int pos;
        binSearch(elem,found,pos);  // Devuelve si el elemento ya estáa y de lo contraario la posición donde debemos insertarlo
        if (!found){
    ...
    // Las operaciones remove tienen un coste de Lineal O(n) siempre,
    // dado que son operaciones sobre el array sin ninguna busqueda
    // Las operaciones get tienen un coste constante O(1) dado que 
    // solo son accesos a los valores sin recorrer el array
    // Eliminamos/devolvemos el ultimo elemento dado que set solo hace inserciones ordenadas
    int getMax() {
        return array[nelems - 1];
    }

    void removeMax() {
        shiftRightFrom(getMax());
        nelems--;
    }

    // Eliminamos/devolvemos el PRIMER elemento dado que set solo hace inserciones ordenadas
    int getMin() {
        return array[0];
    }
